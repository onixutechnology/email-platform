...
const EmailOpensDashboard = () => {
  ...
  const stats = useMemo(() => {
    ...
    const deviceStats = emails.reduce((acc, email) => {
      if (!email.tracking_data) return acc;
      
      if (email.tracking_data.is_mobile) acc.mobile++;
      else if (email.tracking_data.is_tablet) acc.tablet++;
      else acc.desktop++;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: "12px", fontSize: "13px" }}>
                        {email.tracking_data ? (
                          <div>
                            <div>{email.tracking_data.device || "Desconocido"}</div>
                            <div style={{ fontSize: "11px", color: "#9ca3af" }}>
                              {email.tracking_data.os}
                            </div>
                          </div>
